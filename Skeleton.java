...
public class Skeleton {
	...
	// moves the skeleten towards the knight(knights x and y coordinates are in
	// the parameters)
	...
	public boolean checkCollisions(Rectangle knightRect, boolean k) {
		...
		Rectangle skeletonRect = new Rectangle(this.getX() - 10, this.getY() - 10, 28, 28);
		// if it intersects with the knight and it is attacking, the skeleton
		...
		if ((skeletonRect.intersects(knightRect)) && (k == false)) {
			...
			// switch chooses between the top left, top right, bottom left or
			// bottom right corner
			switch (option) {
			case 1:
				this.xcoord = xtemp1;
	...
	// just returns true or false based on where the knight is and if it is
	// intersecting(smaller version of previous method)
